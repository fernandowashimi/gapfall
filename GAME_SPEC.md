...
21. Como jogador, quero que todos os blocos existentes descendam juntos na velocidade de queda atual, para que o estado do tabuleiro seja coerente.
...
- A simulação é contínua em coordenadas de tela: o tabuleiro começa a descer uma altura de bloco por segundo (45 px/s), sobe linearmente até três vezes essa velocidade em 60 segundos de tempo de jogo e depois permanece no teto. Os disparos sobem doze alturas de bloco por segundo (540 px/s). A renderização pode interpolar, mas a simulação é a fonte de verdade.
- A primeira linha inicia no topo e a partida permanece em preparação por três segundos. Após isso, linhas são criadas no topo a cada uma altura de bloco de deslocamento, formando um fluxo contínuo sem espaço vertical. A cadência de geração acompanha a velocidade de queda, não o relógio de parede.
...
- Devem existir testes de geração contínua, composição 3+1 das linhas, limite de repetição de vazio, rampa de velocidade de queda, velocidade de disparo constante e mapeamento de coluna.
...
- Menu de dificuldade ou teto de velocidade escolhido pelo jogador; os knobs de rampa vivem no código.
...
- A rampa de velocidade de queda (tempo de jogo, teto 3×, 60 s, disparo constante) está em ADR-0001 e na spec `.scratch/fall-speed-progression/spec.md`.
